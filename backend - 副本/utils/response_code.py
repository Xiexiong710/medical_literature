...
# ------------------------常用状态码-------------------
def response_200(data: Any = '', message: str = "success") -> Response:
    return ORJSONResponse(status_code=status.HTTP_200_OK, content={'code': 200, 'message': message, 'data': data})


def response_400(code: int = 400, data: str = None, message: str = "请求错误(400)") -> Response:
    return ORJSONResponse(status_code=code, content={'code': 400, 'message': message, 'data': data})


def response_401(code: int = 401, data: str = None, message: str = "未授权，请重新登录(401)") -> Response:
    return ORJSONResponse(status_code=code, content={'code': 401, 'message': message, 'data': data})


def response_403(code: int = 403, data: str = None, message: str = "拒绝访问(403)") -> Response:
    return ORJSONResponse(status_code=code, content={'code': 403, 'message': message, 'data': data})


def response_404(code: int = 404, data: str = None, message: str = "找不到路径(404)") -> Response:
    return ORJSONResponse(status_code=code, content={'code': 404, 'message': message, 'data': data})


def response_422(code: int = 422, data: str = None, message: str = "不可处理的实体(422)") -> Response:
    return ORJSONResponse(status_code=code, content={'code': 422, 'message': message, 'data': data})


def response_500(code: int = 500, data: str = None, message: str = "服务器错误(500)") -> Response:
    return ORJSONResponse(status_code=code, content={'code': 500, 'message': message, 'data': data})


def response_502(code: int = 522, data: str = None, message: str = "网络错误(522)") -> Response:
    return ORJSONResponse(status_code=code, content={'code': 502, 'message': message, 'data': data})